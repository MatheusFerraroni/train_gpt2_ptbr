...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        raw_datasets = raw_datasets.filter(lambda x: x['text'] is not None and len(x['text'])>0)
        ...
        logging.info("-Raw dataset", raw_datasets)
...
#################   #################   ##########          ######################
    ...
    if not os.path.isdir(ByteLevelBPE_tokenizer_pt_rep):
        ...
        logging.info(ByteLevelBPE_tokenizer_pt_vocab_size)
        ...
        if os.path.isdir(ByteLevelBPE_tokenizer_pt_rep):
            shutil.rmtree(ByteLevelBPE_tokenizer_pt_rep)

        os.mkdir(ByteLevelBPE_tokenizer_pt_rep)
        ByteLevelBPE_tokenizer_pt.save_model(str(ByteLevelBPE_tokenizer_pt_rep))
        ByteLevelBPE_tokenizer_pt.save(ByteLevelBPE_tokenizer_pt_rep+"/config.json")

        tokenizer = GPT2TokenizerFast.from_pretrained(
            ByteLevelBPE_tokenizer_pt_rep, 
            pad_token='<|endoftext|>')

        tokenizer.save_pretrained(ByteLevelBPE_tokenizer_pt_rep)
        tokenizer = None
    ...
    if training_args.do_train:
        def splitter(model):
            ...
            return [
                    ...
                    model.transformer.h[4:8],
                    model.transformer.h[8:12],
                    [model.transformer.wte, model.transformer.wpe, model.transformer.ln_f],
            ]

        def freeze_group_model(divs, lyr, status):
            for param in divs[lyr]:
                param.requires_grad_(status)

        def set_all_layer_status_model(divs, status):
            for group in divs:
                for param in group:
                    param.requires_grad_(status)
